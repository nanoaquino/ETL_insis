                ...
                if not chunk_df.empty:
                    ...
                    try:
                        ...
                        for errorObj in cursor.getbatcherrors():
                            i = i+1
                            print(i)
                            item = {"id": errorObj.offset}
                            item['error'] = str(errorObj.message)
                            data.append(item)
                            ...
                            #rejected_df = pd.concat([rejected_df, row_df], axis=0,ignore_index=False, keys=None,levels=None,names=None,verify_integrity=False,sort=False)
                        jsonData=json.dumps(data)
                        json_df = pd.read_json(jsonData)
                        rejected_df = pd.concat([json_df, df_errors], axis=1, join="inner")
                        rejected_df['error_date'] = current_datetime
                        rejected_df['error_type'] = 'Contraint DB'
                        rejected_df.drop('id', axis=1, inplace=True)
